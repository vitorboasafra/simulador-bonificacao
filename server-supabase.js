...
function getLatestPracaWorkbookBackup(workbookPath) {
  return getPracaWorkbookBackupFiles(workbookPath)[0]?.absolute || null;
}
...
function preparePracaWorkbookIfNeeded(workbookPath) {
  ...
  try {
    const backupPath = getLatestPracaWorkbookBackup(absolute);
    ...
    writePracaPrepMeta({
      ...
      backup: backupPath ? path.relative(ROOT_DIR, backupPath) : null,
...
function getInscricaoMatchKeys(rowOrItem) {
  const keys = [
    extractInscricao(rowOrItem),
    normalizeText(rowOrItem.inscricaoEstadual)
  ]
...
function makePracaCentroInscricaoKey({ centro, inscricao }) {
  return [
    normalizeComparable(centro),
    normalizeComparable(cleanNumericCode(inscricao) || inscricao)
  ].join("|||");
}

function getUniquePracaConfig(configs) {
  const validConfigs = configs.filter(config => normalizePracaKey(config.praca));
  const pracaKeys = new Set(validConfigs.map(config => normalizePracaKey(config.praca)));
  return pracaKeys.size === 1 ? validConfigs[0] : null;
}
...
function buildPracaConfigIndex() {
  ...
  const centroInscricaoCandidates = new Map();
  ...
  for (const row of rows) {
    ...
    for (const inscricao of getInscricaoMatchKeys(row)) {
      const centroInscricaoKey = makePracaCentroInscricaoKey({ centro, inscricao });
      if (!centroInscricaoCandidates.has(centroInscricaoKey)) {
        centroInscricaoCandidates.set(centroInscricaoKey, []);
      }
      centroInscricaoCandidates.get(centroInscricaoKey).push(config);
    }
  }

  const byCentroInscricao = new Map();
  for (const [key, configs] of centroInscricaoCandidates.entries()) {
    const uniqueConfig = getUniquePracaConfig(configs);
    if (uniqueConfig) byCentroInscricao.set(key, uniqueConfig);
  }

  pracaConfigCache = { cacheKey, byFull, byCentroInscricao };
...
function getPracaDefaultsForItem(item) {
  ...
  for (const inscricao of getInscricaoMatchKeys(item)) {
    const centroInscricaoKey = makePracaCentroInscricaoKey({ centro: item.centro, inscricao });
    const centroInscricaoConfig = index.byCentroInscricao.get(centroInscricaoKey);
    if (centroInscricaoConfig) return centroInscricaoConfig;
  }
